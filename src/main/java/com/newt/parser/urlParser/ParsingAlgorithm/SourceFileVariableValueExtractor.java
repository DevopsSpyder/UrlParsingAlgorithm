...
/**
 * It prints information extracted from a source file. It is mainly intended as
 * an example usage of JavaSymbolSolver.
 *
 * @author Federico Tomassetti
 */
public class SourceFileVariableValueExtractor {
...
//            } else if ((getParentNode(node) instanceof Statement)) {
            } else if ((getParentNode(node) instanceof VariableDeclarator)) {
//            } else if ((getParentNode(node) instanceof Statement) || (getParentNode(node) instanceof VariableDeclarator)) {\
//            	(getParentNode(node)).
                try {
                    VariableDeclarator vr = (VariableDeclarator) (getParentNode(node));
                    out.println("vr.getNameAsString()" + vr.getNameAsString());
//                    out.println("vr.getType()" + vr.getType().asString());
//System.out.println("vr.getParentNode-->"+vr.getParentNode().get().toString());
                    ok++;
//                    if ("String".equals(vr.getType().asString()) || "StringBuilder".equals(vr.getType().asString()) || "StringBuffer".equals(vr.getType().asString())) {
                        String value = node.toString().replaceAll("\\s+", "");
//                        if (value.contains("\"http://") || value.contains("\"https://")) {
                            if(value.startsWith("\"") || value.startsWith("newString")){
//                            Type ref = JavaParserFacade.get(typeSolver).getType(node);
//                            out.println("  Line " + node.getRange().get().begin.line + ") " + node);//+ " ==> " + ref.describe());
//                            out.println("  Line " + node.getRange().get().begin.line + ") " + node+ " ==> " + ref.describe());
                            int index = value.indexOf("\"");
                            int finalIndex=secondIndexOf(value);
                            if (value.startsWith("\"")) {
                                value = value.substring(index+1,value.length()-1);
                            } else {
//                                value = value.substring(index, value.length() - 1);//replaceAll("newString(", "");
                                value = value.substring(index+1, finalIndex);//replaceAll("newString(", "");
                            }
                            urlList.add(value);
                        }
//                    }
